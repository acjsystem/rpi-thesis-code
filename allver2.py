...
try:	
	...
	def relay_setup():
		...
		GPIO.setup(gsmrestart, GPIO.OUT)
		GPIO.output(relay_pin,GPIO.HIGH)
		GPIO.output(gsmrestart,GPIO.HIGH)
	...
	def get_user_number(): #create method where if net stops in the middle of sending data it cancels
		...
		try:
			response = requests.post(url=url, data=req_info, timeout=10)

		except requests.exceptions.ConnectionError:
			print ("Not connected timeout or no error - get user num")
	...
	def get_car_stat():
		...
		try:
			...
			print ("Not connected timeout or no error - get carstat")
		else:
	...
	def post_report_stat(): #Add if storage_id!= prev
		...
		if sms_ignition !="" or sms_gps != "":
			...
			try:
				...
				print ("Not connected timeout or no error - post report stat")
			else:		
				#print (response.json())
				...
				if response.json()['Error'] == "True":
					...
					if car_stat == "True":	
						...
						try:
							...
							print ("Not connected timeout or no error - post report addreport")
						#print (response.json())
						else:
							sms_ignition =""
							sms_gps = ""
